...
在非简单开发开始前调查相关代码、测试和文档，只追问真正会改变产品行为、契约、范围或验收结果的问题。它先进行不落盘的轻量判定；机械修改、明确的低风险任务和仅涉及局部实现选择的工作会直接退出，不创建 `brief.md`。隐式发现实质未决决策时，会说明证据和影响、建议是否进入结构化澄清并等待用户确认，确认前不落盘。用户明确要求澄清需求、逐项确认、一次问一个问题、先明确需求、共同定义功能行为或边界，或者创建 brief 时，视为已授权进入，不重复询问。简报完成后，Skill 会自我复查、等待用户确认、报告澄清完成，然后停止，不调用或依赖其他 Skill。

适合：

- 将“一句话需求”整理成实现就绪的 brief；
- 在用户要求逐项确认时，一次澄清一个会改变决策的问题；
- 明确目标、范围、非目标和验收标准；
- 澄清状态、失败语义、兼容性、安全和副作用边界；
- 独立解决会影响后续工作的未决问题，不依赖其他 Skill。
...
用户确认 `brief.md` 后，Skill 会停止并等待用户选择下一步；它不会自动进入规格、规划或实现，也不依赖其他 Skill。
